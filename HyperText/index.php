<?php 
if(!isset($_SESSION)) {
    session_start();
}

include('../Formulário/Login/protecao.php');

?>


<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../Estilo/index-style.css">
    <link rel="shortcut icon" href="../controle.ico" type="image/x-icon">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-iecdLmaskl7CVkqkXNQ/ZH/XLlvWZOJyj7Yy7tcenmpD1ypASozpmT/E0iPtmFIB46ZmdtAc9eNBvH0H/ZpiBw==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <title>Jornal</title>
</head>
<body>
    <header id="header">
        <img src="../Imagens/logo.png" alt="Logo">
    </header>

    <nav id="nav">
        <ul id="menu">
            <li> <a href="mibr.html" target="_blank" id="mibr">MIBR</a> </li>
            <li> <a href="furia.html" target="_blank" id="furia">FURIA</a> </li>
            <li> <a href="imperial.html" target="_blank" id="imperial">Imperial</a> </li>
            <li> <a href="sk.html" target="_blank" id="sk">SK</a> </li>
            <li> <a href="../Formulário/Login/logout.php" id="sair">SAIR</a> </li>
        </ul>

        <div class="mudarTema">
            <button id="btn-dark-mode-toggle" title="botaoTema">OK</button>
        </div>
    </nav>

    <div class="container">
        <button class="arrow-left control" aria-label="Previous image">◀</button>
        <button class="arrow-right control" aria-label="Next Image">▶</button>
        <div class="noticias-wrapper carrousel">
            <div class="noticias">
                <p><img src="../Imagens/Furia/fallen-contratacao.jpeg" alt="Contratação do fallen" class="item current-item"/>FURIA contrata FalleN</p> 
                <p><img src="../Imagens/Furia/especulacao-fallen.jpg" alt="Especulação da contratação do fallen" class="item">Há especulações sobre a contratação de FalleN</p>
                <p><img src="../Imagens/Furia/rio-major.webp" alt="Major Rio de Janeiro" class="item">FURIA ganha da NAVI no Rio de Janeiro</p>
                <p><img src="../Imagens/Furia/saida-saffe.jpg" alt="Saída do saffe" class="item">Saida de saffe é confirmada pela FURIA</p>
                <p><img src="../Imagens/Imperial/the-last-dance.jpg" alt="The Last Dance" class="item">Imperial anuncia The Last Dance</p>
                <p><img src="../Imagens/Imperial/imperial-ultima-vaga-rmr.jpg" alt="Última vaga RMR" class="item">Imperial conquista última vaga para o Major</p>
                <p><img src="../Imagens/Imperial/saida-fer-imperial.webp" alt="Saída do Fer da Imperial" class="item">Fer anuncia saida da Imperial</p>
                <p><img src="..//Imagens/MIBR/hen1-banco.webp" alt="HEN1 no banco de reservas" class="item">MIBR anuncia saida de HEN1</p>
                <p><img src="../Imagens/MIBR/Tuurtle-banco.jpg" alt="Tuurtle no banco de reservas" class="item">Tuurtle se despede do MIBR</p>
                <p><img src="../Imagens/SK/sk-major-one-cologne-2016.jpg" alt="SK vence Major em Cologne em 2016" class="item">Brasil conquista o mundo mais uma vez</p>
                <p><img src="../Imagens/SK/epl-odense-2017-sk.jpg" alt="SK vence ESL Pro League Odense 2017" class="item">SK é campeã da EPL Odense 2017</p>
                <p><img src="../Imagens/SK/blast-trofeu.jpg" alt="Troféu BLAST" class="item">Brasileiros conquistam a Blast e calam a Dinamarca</p>
            </div>
        </div>
    </div>

    <main>
    <article id="grid-noticias-principais">
        <div id="imagem-principal"><a href="furia.html" target="_blank"><img src="../Imagens/Furia/furia-contratacao-fallen.jpg" alt="Fallen é contratado"/></a>

        <h2>Fallen é contratado pela Furia Sua estreia será no dia 26 de julho pelo play-in da IEM Cologne 2023</h2> 
        </div>

        <div id="imagem-secundaria-1"><a href="sk.html" target="_blank"><img src="../Imagens/SK/sk-disband.jpg" alt="Há 6 anos a line-up da SK dava disband"/></a>

        <h2>Há 5 anos SK Gaming anunciava disband e MIBR comprava line-up</h2>
        </div>

        <div id="imagem-secundaria-2"><a href="furia.html" target="_blank"><img src="../Imagens/Furia/saffe-drop-de-saida.jpg" alt="Saffe e drop saem da Fúria"></a>

        <h2>Saffe e Drop vão para o banco da FURIA!</h2>
        </div>

        <div id="borda-linha"></div>
    </article>

    <div class="container-flex">
    <article id="grid-mibr">
        <h2 id="secao">MIBR</h2>

        <div class="mibr-noticia-principal">
           <a href="MIBR/mibr-anuncio.html"><img src="../Imagens/MIBR/mibr-drop-saffe.jpg" alt="Mibr Anuncia Drop e Saffe"/></a>
           <h3>MIBR oficializa contratação de Saffe e Drop! A dupla estreia no dia 16 de agosto</h3>
        </div>

        <div class="mibr-noticia-secundaria-1">
            <a href="MIBR/mibr-tuurtle.html"><img src="../Imagens/MIBR/Tuurtle-banco.jpg" alt="Tuurtle vai para o banco do MIBR"></a>
            <h3>Tuurtle é colocado no banco do MIBR</h3>
        </div>

        <div class="mibr-noticia-secundaria-2">
            <a href="MIBR/mibr-hen1.html"><img src="../Imagens/MIBR/hen1-banco.webp" alt="HEN1 vai para o banco do MIBR"></a>
            <h3>HEN1 é pego de surpresa e vai ao banco do MIBR</h3>
        </div>
    </article>

    <article id="anuncio">
    </article>
    </div>

    <div class="container-flex">
    <article id="grid-furia">
        <h2 id="secao-furia">FURIA</h2>

        <div class="furia-noticia-principal">
            <a href="FURIA/furia-perde.html" target="_blank"><img src="../Imagens/Furia/furia-perde.webp" alt="Furia perde para Heroic e está fora do Major Rio"></a>

            <h3>Em um jogo acirrado a FURIA perde por 2x1 e está eliminada do IEM Rio Major 2022</h3>
        </div>

        <div class="furia-noticia-secundaria-1">
            <a href="FURIA/furia-major.html" target="_blank"><img src="../Imagens/Furia/rio-major.webp" alt="Furia ganha da NAVI e avança as semifinais do Major Rio"></a>

            <h3>FURIA ganha da NAVI de virada em um jogo histórico no Rio de Janeiro</h3>
        </div>

        <div class="furia-noticia-secundaria-2">
            <a href="FURIA/furia-parceria.html" target="_blank"><img src="../Imagens/Furia/furia-parceria-esl.webp" alt="Furia assina contrato e vira a nova parceira da ESL"></a>

            <h3>FURIA assina o Louvre Agreement e se torna a nova parceira da ESL</h3>
        </div>
    </article>

    <article id="proximos-eventos">
        <div id="secao-eventos">Próximos Eventos</div>

        <div class="informacoes-evento">
            <div class="logo-evento">
                <img src="../Imagens/logos-eventos/gamers8.png" alt="Logo Gamers8">
            </div>

        <div class="tabela">
            <table class="table">
                <tbody>
                    <tr>
                        <td class="nome-evento">
                            <div>Gamers8 2023</div>
                        </td>

                        <td class="n-times">16</td>

                        <td class="prizepool">$1.000.000</td>

                        <td class="atividade">LAN</td>
                    </tr>

                    <tr class="eventoDetalhes">
                        <td>
                            <span class="pais-evento">
                                <img src="../Imagens/paises/as.webp" alt="Arábia Saudita">
                                <span class="col-desc">Riyadh, Arábia Saudita |</span>
                            </span>
                            <span class="col-desc">
                                <span>
                                    <span>1 Ago</span>
                                    <span>
                                        -
                                        <span>26 Ago</span>
                                    </span>
                                </span>
                            </span>
                        </td>
                        <td class="col-desc">Teams</td>
                        <td class="col-desc">Prize</td>
                        <td class="col-desc"></td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>

    <div class="informacoes-evento-2">
        <div class="logo-evento">
            <img src="../Imagens/logos-eventos/epl.svg" alt="Logo EPL">
        </div>

    <div class="tabela">
        <table class="table">
            <tbody>
                <tr>
                    <td class="nome-evento">
                        <div>ESL Pro League Season 18</div>
                    </td>

                    <td class="n-times">32</td>

                    <td class="prizepool">$850.000</td>

                    <td class="atividade">LAN</td>
                </tr>

                <tr class="eventoDetalhes">
                    <td>
                        <span class="pais-evento">
                            <img src="../Imagens/paises/mt.webp" alt="Malta">
                            <span class="col-desc">Malta |</span>
                        </span>
                        <span class="col-desc">
                            <span>
                                <span>30 Ago</span>
                                <span>
                                    -
                                    <span>1 Out</span>
                                </span>
                            </span>
                        </span>
                    </td>
                    <td class="col-desc">Teams</td>
                    <td class="col-desc">Prize</td>
                    <td class="col-desc"></td>
                </tr>
            </tbody>
        </table>
    </div>
</div>

<div class="informacoes-evento">
    <div class="logo-evento">
        <img src="../Imagens/logos-eventos/hussar.webp" alt="Logo Hussar">
    </div>

<div class="tabela">
    <table class="table">
        <tbody>
            <tr>
                <td class="nome-evento">
                    <div>Hussar Cup 2023</div>
                </td>

                <td class="n-times">8</td>

                <td class="prizepool">$1.000.000</td>

                <td class="atividade">Intl. LAN</td>
            </tr>

            <tr class="eventoDetalhes">
                <td>
                    <span class="pais-evento">
                        <img src="../Imagens/paises/pl.webp" alt="Polônia">
                        <span class="col-desc">Chorzów, Polônia |</span>
                    </span>
                    <span class="col-desc">
                        <span>
                            <span>30 Ago</span>
                            <span>
                                -
                                <span>3 Set</span>
                            </span>
                        </span>
                    </span>
                </td>
                <td class="col-desc">Teams</td>
                <td class="col-desc">Prize</td>
                <td class="col-desc"></td>
            </tr>
        </tbody>
    </table>
</div>
</div>

<div class="informacoes-evento-2">
    <div class="logo-evento">
        <img src="../Imagens/logos-eventos/blast.webp" alt="Logo BLAST">
    </div>

<div class="tabela">
    <table class="table">
        <tbody>
            <tr>
                <td class="nome-evento">
                    <div>BLAST Premier Fall Showdown 2023</div>
                </td>

                <td class="n-times">16</td>

                <td class="prizepool">$135.000</td>

                <td class="atividade">Online</td>
            </tr>

            <tr class="eventoDetalhes">
                <td>
                    <span class="pais-evento">
                        <img src="../Imagens/paises/world.webp" alt="Mundo">
                        <span class="col-desc">TBA |</span>
                    </span>
                    <span class="col-desc">
                        <span>
                            <span>4 Out</span>
                            <span>
                                -
                                <span>8 Out</span>
                            </span>
                        </span>
                    </span>
                </td>
                <td class="col-desc">Teams</td>
                <td class="col-desc">Prize</td>
                <td class="col-desc"></td>
            </tr>
        </tbody>
    </table>
</div>
</div>

<div class="informacoes-evento">
    <div class="logo-evento">
        <img src="../Imagens/logos-eventos/iem.png" alt="Logo IEM">
    </div>

<div class="tabela">
    <table class="table">
        <tbody>
            <tr>
                <td class="nome-evento">
                    <div>IEM Sydney 2023</div>
                </td>

                <td class="n-times">16</td>

                <td class="prizepool">$250.000</td>

                <td class="atividade">LAN</td>
            </tr>

            <tr class="eventoDetalhes">
                <td>
                    <span class="pais-evento">
                        <img src="../Imagens/paises/au.webp" alt="Austrália">
                        <span class="col-desc">Sydney, Austrália |</span>
                    </span>
                    <span class="col-desc">
                        <span>
                            <span>16 Out</span>
                            <span>
                                -
                                <span>22 Out</span>
                            </span>
                        </span>
                    </span>
                </td>
                <td class="col-desc">Teams</td>
                <td class="col-desc">Prize</td>
                <td class="col-desc"></td>
            </tr>
        </tbody>
    </table>
</div>
</div>

<div class="informacoes-evento-2">
    <div class="logo-evento">
        <img src="../Imagens/logos-eventos/blast.webp" alt="Logo BLAST">
    </div>

<div class="tabela">
    <table class="table">
        <tbody>
            <tr>
                <td class="nome-evento">
                    <div>BLAST Premier Fall Final 2023</div>
                </td>

                <td class="n-times">8</td>

                <td class="prizepool">$425.000</td>

                <td class="atividade">LAN</td>
            </tr>

            <tr class="eventoDetalhes">
                <td>
                    <span class="pais-evento">
                        <img src="../Imagens/paises/di.webp" alt="Dinamarca">
                        <span class="col-desc">Copenhagen, Dinamarca |</span>
                    </span>
                    <span class="col-desc">
                        <span>
                            <span>22 Nov</span>
                            <span>
                                -
                                <span>26 Nov</span>
                            </span>
                        </span>
                    </span>
                </td>
                <td class="col-desc">Teams</td>
                <td class="col-desc">Prize</td>
                <td class="col-desc"></td>
            </tr>
        </tbody>
    </table>
</div>
</div>

<div class="informacoes-evento">
    <div class="logo-evento">
        <img src="../Imagens/logos-eventos/fireleague.webp" alt="Logo FiReLEAGUE">
    </div>

<div class="tabela">
    <table class="table">
        <tbody>
            <tr>
                <td class="nome-evento">
                    <div>FiReLEAGUE 2023 Global Finals</div>
                </td>

                <td class="n-times">8</td>

                <td class="prizepool">$150.000</td>

                <td class="atividade">Intl. LAN</td>
            </tr>

            <tr class="eventoDetalhes">
                <td>
                    <span class="pais-evento">
                        <img src="../Imagens/paises/es.webp" alt="Espanha">
                        <span class="col-desc">Barcelona, Espanha |</span>
                    </span>
                    <span class="col-desc">
                        <span>
                            <span>8 Nov</span>
                            <span>
                                -
                                <span>12 Nov</span>
                            </span>
                        </span>
                    </span>
                </td>
                <td class="col-desc">Teams</td>
                <td class="col-desc">Prize</td>
                <td class="col-desc"></td>
            </tr>
        </tbody>
    </table>
</div>
</div>

<div class="informacoes-evento-2">
    <div class="logo-evento">
        <img src="../Imagens/logos-eventos/blast-global.webp" alt="Logo BLAST Global">
    </div>

<div class="tabela">
    <table class="table">
        <tbody>
            <tr>
                <td class="nome-evento">
                    <div>BLAST Premier World Final 2023</div>
                </td>

                <td class="n-times">8</td>

                <td class="prizepool">$1.000.000</td>

                <td class="atividade">LAN</td>
            </tr>

            <tr class="eventoDetalhes">
                <td>
                    <span class="pais-evento">
                        <img src="../Imagens/paises/eau.webp" alt="EAU">
                        <span class="col-desc">Abu Dhabi, EAU |</span>
                    </span>
                    <span class="col-desc">
                        <span>
                            <span>13 Dez</span>
                            <span>
                                -
                                <span>17 Dez</span>
                            </span>
                        </span>
                    </span>
                </td>
                <td class="col-desc">Teams</td>
                <td class="col-desc">Prize</td>
                <td class="col-desc"></td>
            </tr>
        </tbody>
    </table>
</div>
</div>
    </article>
    </div>

    <div class="container-flex">
    <article id="grid-imperial">
        <h2 id="secao-imperial">Imperial</h2>

        <div class="imp-noticia-principal">
        <a href="Imperial/contratacao-imp.html" target="_blank"><img src="../Imagens/Imperial/contratacao-hf.webp" alt="Imperial contrata HEN1 e Felps"></a>

        <h3>Imperial anuncia a contratação de HEN1 e Felps</h3>
        </div>

        <div class="imp-noticia-secundaria-1">
            <a href="Imperial/ultima-vaga.html" target="_blank"><img src="../Imagens/Imperial/imperial-ultima-vaga-rmr.jpg" alt="Imperial conquista a última vaga do RMR"></a>

            <h3>Imperial conquista a última vaga do RMR e irá ao Major</h3>
        </div>

        <div class="imp-noticia-secundaria-2">
            <a href="Imperial/last-dance.html" target="_blank"><img src="../Imagens/Imperial/the-last-dance.jpg" alt="The Last Dance é anunciado"></a>

            <h3>Imperial anuncia o The Last Dance</h3>
        </div>
    </article>

    <article id="ranking-hltv">
        <div id="secao-ranking">Ranking HLTV</div>

        <div id="ranking-holder">
        <div class="informacoes-ranking">

            <span class="posicao-ranking">#17</span>

            <span class="logo-ranking">
                <img src="../Imagens/Imperial/logo-imperial.png" alt="Logo Imperial">
            </span>

            <div class="relative">
                <div class="nome-pontos">
                    <span class="nomeTime">Imperial</span>
                    <span class="pontosTime">(122 pontos)</span>
                </div>

                <div class="linha-players">
                    <div class="ranking-nick-nomes">
                        <span>HEN1 •</span>
                    </div>
                    <div class="ranking-nick-nomes">
                        <span>boltz •</span>
                    </div>
                    <div class="ranking-nick-nomes">
                        <span>felps •</span>
                    </div>
                    <div class="ranking-nick-nomes">
                        <span>VINI •</span>
                    </div>
                    <div class="ranking-nick-nomes">
                        <span>JOTA</span>
                    </div>
                </div>
            </div>

            <span class="enchimento"></span>

            <div class="mudanca-negativa">-1</div>
        </div>

            <div class="lineup-con">
                <table class="lineup">
                    <tbody>
                        <tr>
                            <td class="player-holder">
                                <img src="../Imagens/Imperial/Jogadores/hen1.webp" alt="HEN1">

                                <div class="nick">
                                    <img src="../Imagens/paises/br.webp" alt="Brasil"> HEN1
                                </div>
                            </td>
                            <td class="player-holder">
                                <img src="../Imagens/Imperial/Jogadores/boltz.webp" alt="boltz">

                                <div class="nick">
                                    <img src="../Imagens/paises/br.webp" alt="Brasil"> boltz
                                </div>
                            </td>
                            <td class="player-holder">
                                <img src="../Imagens/Imperial/Jogadores/felps.webp" alt="felps">

                                <div class="nick">
                                    <img src="../Imagens/paises/br.webp" alt="Brasil"> felps
                                </div>
                            </td>
                            <td class="player-holder">
                                <img src="../Imagens/Imperial/Jogadores/vini.webp" alt="VINI">

                                <div class="nick">
                                    <img src="../Imagens/paises/br.webp" alt="Brasil"> VINI
                                </div>
                            </td>
                            <td class="player-holder">
                                <img src="../Imagens/Imperial/Jogadores/jota.webp" alt="JOTA">

                                <div class="nick">
                                    <img src="../Imagens/paises/br.webp" alt="Brasil"> JOTA
                                </div>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>

            <div class="informacoes-ranking">

                <span class="posicao-ranking">#19</span>
    
                <span class="logo-ranking">
                    <img src="../Imagens/Furia/logo-furia.png" alt="Logo FURIA">
                </span>
    
                <div class="relative">
                    <div class="nome-pontos">
                        <span class="nomeTime">FURIA</span>
                        <span class="pontosTime">(109 pontos)</span>
                    </div>
    
                    <div class="linha-players">
                        <div class="ranking-nick-nomes">
                            <span>FalleN •</span>
                        </div>
                        <div class="ranking-nick-nomes">
                            <span>chelo •</span>
                        </div>
                        <div class="ranking-nick-nomes">
                            <span>arT •</span>
                        </div>
                        <div class="ranking-nick-nomes">
                            <span>yuurih •</span>
                        </div>
                        <div class="ranking-nick-nomes">
                            <span>KSCERATO</span>
                        </div>
                    </div>
                </div>
    
                <span class="enchimento"></span>
    
                <div class="mudanca-negativa">-1</div>
                </div>

                <div class="lineup-con">
                    <table class="lineup">
                        <tbody>
                            <tr>
                                <td class="player-holder">
                                    <img src="../Imagens/Furia/Jogadores/fallen.webp" alt="Fallen">
    
                                    <div class="nick">
                                        <img src="../Imagens/paises/br.webp" alt="Brasil"> FalleN
                                    </div>
                                </td>
                                <td class="player-holder">
                                    <img src="../Imagens/Furia/Jogadores/chelo.webp" alt="chelo">
    
                                    <div class="nick">
                                        <img src="../Imagens/paises/br.webp" alt="Brasil"> chelo
                                    </div>
                                </td>
                                <td class="player-holder">
                                    <img src="../Imagens/Furia/Jogadores/art.webp" alt="art">
    
                                    <div class="nick">
                                        <img src="../Imagens/paises/br.webp" alt="Brasil"> arT
                                    </div>
                                </td>
                                <td class="player-holder">
                                    <img src="../Imagens/Furia/Jogadores/yuurih.webp" alt="yuurih">
    
                                    <div class="nick">
                                        <img src="../Imagens/paises/br.webp" alt="Brasil"> yuurih
                                    </div>
                                </td>
                                <td class="player-holder">
                                    <img src="../Imagens/Furia/Jogadores/kscerato.webp" alt="kscerato">
    
                                    <div class="nick">
                                        <img src="../Imagens/paises/br.webp" alt="Brasil"> KSCERATO
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>

                <div class="informacoes-ranking">

                    <span class="posicao-ranking">#33</span>
        
                    <span class="logo-ranking">
                        <img src="../Imagens/MIBR/logo-secundaria.png" alt="Logo MIBR">
                    </span>
        
                    <div class="relative">
                        <div class="nome-pontos">
                            <span class="nomeTime">MIBR</span>
                            <span class="pontosTime">(31 pontos)</span>
                        </div>
        
                        <div class="linha-players">
                            <div class="ranking-nick-nomes">
                                <span>exit •</span>
                            </div>
                            <div class="ranking-nick-nomes">
                                <span>saffe •</span>
                            </div>
                            <div class="ranking-nick-nomes">
                                <span>drop •</span>
                            </div>
                            <div class="ranking-nick-nomes">
                                <span>brnz4n •</span>
                            </div>
                            <div class="ranking-nick-nomes">
                                <span>insani</span>
                            </div>
                        </div>
                    </div>
        
                    <span class="enchimento"></span>
        
                    <div class="mudanca-negativa">-2</div>
                    </div>

                    <div class="lineup-con">
                        <table class="lineup">
                            <tbody>
                                <tr>
                                    <td class="player-holder">
                                        <img src="../Imagens/MIBR/Jogadores/exit.webp" alt="exit">
        
                                        <div class="nick">
                                            <img src="../Imagens/paises/br.webp" alt="Brasil"> exit
                                        </div>
                                    </td>
                                    <td class="player-holder">
                                        <img src="../Imagens/MIBR/Jogadores/saffe.webp" alt="saffe">
        
                                        <div class="nick">
                                            <img src="../Imagens/paises/br.webp" alt="Brasil"> saffe
                                        </div>
                                    </td>
                                    <td class="player-holder">
                                        <img src="../Imagens/MIBR/Jogadores/drop.webp" alt="drop">
        
                                        <div class="nick">
                                            <img src="../Imagens/paises/br.webp" alt="Brasil"> drop
                                        </div>
                                    </td>
                                    <td class="player-holder">
                                        <img src="../Imagens/MIBR/Jogadores/brnz4n.webp" alt="brnzan">
        
                                        <div class="nick">
                                            <img src="../Imagens/paises/br.webp" alt="Brasil"> brnz4n
                                        </div>
                                    </td>
                                    <td class="player-holder">
                                        <img src="../Imagens/MIBR/Jogadores/insani.webp" alt="insani">
        
                                        <div class="nick">
                                            <img src="../Imagens/paises/br.webp" alt="Brasil"> insani
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
        </div>
    </div>
    </article>
    </div>

    <div class="container-flex">
    <article id="grid-sk">

    <h2 id="secao-sk">SK Gaming</h2>

    <div class="sk-noticia-principal">
        <a href="SK/cologne.html" target="_blank"><img src="../Imagens/SK/sk-major-one-cologne-2016.jpg" alt="Brasileiros conquistam segundo Major seguido"></a>

        <h3>Brasileiros conquistam o mundo pela segunda vez seguida e se tornam o melhor time de CSGO</h3>
    </div>

    <div class="sk-noticia-secundaria-1">
        <a href="SK/epl.html" target="_blank"><img src="../Imagens/SK/epl-odense-2017-sk.jpg" alt="SK ganha ESL Pro League Odense 2017"></a>

        <h3>SK Gaming conquista a ESL Pro League Odense 2017</h3>
    </div>

    <div class="sk-noticia-secundaria-2">
        <a href="SK/epicenter.html" target="_blank"><img src="../Imagens/SK/epicenter-2016.jpg" alt="SK conquista EPICENTER 2016"></a>

        <h3>Em uma partida emocionante SK Gaming conquista EPICENTER 2017 por 3x2</h3>
    </div>
    </article>

    <article id="anuncio-2">
    </article>
    </div>

    <footer>
        <div class="footerContainer">
        <div class="redes-sociais">
            <a href="https://github.com/Joaovictor23t4" target="_blank"><i class="fa-brands fa-github"></i></a>
            <a href="https://twitter.com/ParthTK1" target="_blank"><i class="fa-brands fa-twitter"></i></a>
            <a href="https://www.instagram.com/joao_tk_victor13/" target="_blank"><i class="fa-brands fa-instagram"></i></a>
            <a href="https://www.linkedin.com/in/jo%C3%A3o-victor-portela-rocha-a59886279/" target="_blank"><i class="fa-brands fa-linkedin"></i></a>
        </div>

        <div class="navFooter">
            <ul>
                <li><a href="">Home</a></li>
                <li><a href="">Melhores Momentos Brasil</a></li>
                <li><a href="">Sobre</a></li>
            </ul>
        </div>
    </div>

    <div class="copyright">
            <p>Copyright &copy; 2023 <span class="designed"> Feito por João Victor Portela Rocha</span></p>
        </div>
    </footer>
    </main>
    <script src="../JavaScript/index.js"></script>
</body>
</html>